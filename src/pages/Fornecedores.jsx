...
import { maskDoc, fone, uid } from '../lib/format.js'
import {
...
export default function Fornecedores() {
  ...
  const list = useMemo(() => {
    return (db.fornecedores || []).filter((c) => {
      ...
      const txt = `${c.razaoSocial} ${c.nomeFantasia} ${c.cpfCnpj}`.toLowerCase()
      ...
      <Card>
        <div className="card-head">
          <div className="search" style={{ maxWidth: 320, background: 'var(--surface-2)', border: '1px solid var(--border)', borderRadius: 9, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 8 }}>
            ...
            <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar por nome, CNPJ/CPF..." style={{ border: 'none', background: 'transparent', outline: 'none', flex: 1 }} />
          </div>
        ...
        <div className="table-wrap">
          <table className="tbl">
            <thead>
              <tr>
                <th>Fornecedor</th><th>Tipo</th><th>Contato</th><th>Status</th>
              </tr>
            </thead>
            <tbody>
              {list.map((c) => (
